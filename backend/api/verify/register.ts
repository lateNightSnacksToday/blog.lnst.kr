...
const Handler: Backend["handler"] = async (req, res) => {
  ...
  if (!email)
    return res
      .status(400)
      .json({ success: false, status: -203, message: "Email is required" });
  ...
  if (!nickname)
    return res
      .status(400)
      .json({ success: false, status: -203, message: "Nickname is required" });
  if (!validateName(nickname)) {
    return res
      .status(400)
      .json({ success: false, status: -203, message: "Invalid nickname." });
  }

  if (
    !email.endsWith("@gsm.hs.kr") || email.startsWith("s")
      ? isNaN(email.slice(1).split("@gsm.hs.kr")[0])
      : true
  ) {
    return res.status(400).json({
      success: false,
      status: -308,
      message: "Only GSM email addresses are allowed",
    });
  }
  ...
  const isExistingNickname =
    Verify.isExistingVerificationWithNickname(nickname);
